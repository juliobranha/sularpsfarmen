...
function themefusion_breadcrumb() {
        ...
        if(is_singular('avada_portfolio')) {
            echo get_the_term_list(get_queried_object_id(), 'portfolio_category', '<li>', '&nbsp;/&nbsp;&nbsp;', '</li>');
            echo '<li>'.get_the_title(get_queried_object_id()).'</li>';
        }

        if(is_singular('event')) {
            $terms = get_the_term_list(get_queried_object_id(), 'event-categories', '<li>', '&nbsp;/&nbsp;&nbsp;', '</li>');
            if( ! is_wp_error( $terms ) ) {
                echo get_the_term_list(get_queried_object_id(), 'event-categories', '<li>', '&nbsp;/&nbsp;&nbsp;', '</li>');
            }
            echo '<li>'.get_the_title(get_queried_object_id()).'</li>';
        }
        ...
        if(is_page() && !is_front_page()) {
            $parents = array();
            $get_post = get_post( get_queried_object_id() );
        ...
        if(is_single() && !is_singular('avada_portfolio')  && !is_singular('event')) {
            $categories_1 = get_the_category(get_queried_object_id());
            if($categories_1):
