...
if( ! class_exists( 'FusionTemplateWoo' ) ) {
    ...
    class FusionTemplateWoo {
		...
		function after_shop_item_buttons() {
			echo '<a href="' . get_permalink() . '" class="show_details_button">' . __( 'Show details', 'Avada' ) . '</a></div></div>';
		}
...
if( ! $data['woocommerce_avada_ordering'] ) {
...
function avada_woocommerce_catalog_ordering() {
	...
	$html .= '<li>';
	$html .= '<span class="current-li"><span class="current-li-content"><a>'.__('Sort by', 'Avada').' <strong>'.__('Default Order', 'Avada').'</strong></a></span></span>';
	$html .= '<ul>';
...
function avada_woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	$fragments['#header-sticky .cart'] = $header_cart;
...
function avada_woocommerce_after_single_product_summary()
{
	...
	if($data['woocommerce_social_links']) {
		$social .= '<ul class="social-share">
			<li class="facebook">
				<a href="http://www.facebook.com/sharer.php?m2w&s=100&p&#91;url&#93;=' . get_permalink() . '&p&#91;title&#93;=' . get_the_title() .'" target="_blank"' . $nofollow .'>				
...
function avada_woocommerce_cart_collaterals($args)
{
	...
	if ( WC()->cart->coupons_enabled() ) { ?>
		<div class="coupon">
			...
			<input type="submit" class="button small default" name="apply_coupon" value="<?php _e( 'Apply', 'Avada' ); ?>" />
...
function avada_woocommerce_after_cart_totals($args)
{
	...
	<input type="submit" class="button small default" name="update_cart" value="<?php _e( 'Update Cart', 'woocommerce' ); ?>" />
	<input type="submit" class="checkout-button button small default alt wc-forward" name="proceed" value="<?php _e( 'Proceed to Checkout', 'woocommerce' ); ?> &rarr;" />
...
if( ! $data['woocommerce_one_page_checkout'] ) {
...
function avada_woocommerce_checkout_billing($args)
{
	...
	<a data-name="<?php echo $data_name; ?>" href="#" class="default button small continue-checkout"><?php _e('Continue', 'Avada'); ?></a>
	<div class="clearboth"></div>
...
function avada_woocommerce_checkout_shipping($args)
{
	...
	<a data-name="#order_review" href="#" class="default button small continue-checkout"><?php _e('Continue', 'Avada'); ?></a>
	<div class="clearboth"></div>
...
function avada_enable_order_notes_field() {
	...
	if( ! $data['woocommerce_enable_order_notes'] ) {
		return 0;
	}
...
function avada_woocommerce_before_my_account($edit_address = false)
{

	global $data, $woocommerce, $current_user;
	?>
