...
function tf_main_menu_args($args) {
	...
	if ( get_post_meta($c_pageID, 'pyre_displayed_menu', true) != 'default' && get_post_meta($c_pageID, 'pyre_displayed_menu', true) != '' &&
...
if(! $data['disable_megamenu']) {
	...
	function create_avada_menu() {
		...
		$main_menu = wp_nav_menu(array('theme_location' => 'main_navigation', 'depth' => 5, 'container' => false, 'menu_id' => 'nav',  'items_wrap' => '%3$s', 'echo' => false));
	}
}
...
function tf_content($limit, $strip_html) {
	...
	if($test_strip_html) {
		...
		if($post->post_excerpt || $pos !== false) {
			$more = 0;
			$raw_content = strip_tags( get_the_excerpt($readmore) );
			$custom_excerpt = true;
		}
	} else {
		...
		if($post->post_excerpt || $pos !== false) {
			$more = 0;
			$raw_content = get_the_excerpt($readmore);
			$custom_excerpt = true;
		}
	}
	...
	if($raw_content && $custom_excerpt == false) {
		...
		if( $data['excerpt_base'] == 'Characters' ) {
			$content = mb_substr($content, 0, $limit);
			if($limit != 0 && !$data['disable_excerpts']) {
				$content .= ' &#91;...&#93;';
			}
		} else {
...
function avada_scripts() {
	...
	if(isset($post)) {
		$slider_page_id = get_queried_object_id();
	}
	...
	if ( class_exists( 'Woocommerce' ) ) {
		if(!$data['status_lightbox'] && !is_woocommerce() || !$data['status_lightbox'] && is_woocommerce() && is_shop()) {
...
function avada_get_the_password_form() {
	...
	<p><label for="' . $label . '">' . __( 'Password:' ) . ' <input class="password" name="post_password" id="' . $label . '" type="password" size="20" /></label> <input class="button small default"  type="submit" name="Submit" value="' . esc_attr__( 'Submit' ) . '" /></p></form>
	';
	return $output;
}
